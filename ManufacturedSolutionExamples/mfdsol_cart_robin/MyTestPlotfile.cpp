...
MyTest::writePlotfile () const
{
    ...
    if (gpu_regtest) {
        const int ncomp = (acoef.empty()) ? 7 : 9;
        Vector<std::string> varname = {"solution", "rhs", "exact_solution", "error", "robinbc_a", "robinbc_b", "robinbc_f"};
        if (!acoef.empty()) {
        ...
        for (int ilev = 0; ilev < nlevels; ++ilev)
        {
            ...
            MultiFab::Copy(plotmf[ilev], robinbc_a     [ilev], 0, 4, 1, 0);
            MultiFab::Copy(plotmf[ilev], robinbc_b     [ilev], 0, 5, 1, 0);
            MultiFab::Copy(plotmf[ilev], robinbc_f     [ilev], 0, 6, 1, 0);
            if (!acoef.empty()) {
                MultiFab::Copy(plotmf[ilev], acoef[ilev], 0, 7, 1, 0);
                MultiFab::Copy(plotmf[ilev], bcoef[ilev], 0, 8, 1, 0);
            }
        }
