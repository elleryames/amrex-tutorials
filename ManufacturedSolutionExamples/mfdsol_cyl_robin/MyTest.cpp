...
MyTest::solve ()
{
    ...
    if (composite_solve)
    {
        ...
        // This is a 2d problem with Robin BC
        mlabec.setDomainBC({LinOpBCType::Neumann,
                            LinOpBCType::Robin},
                            {LinOpBCType::Robin,
                                LinOpBCType::Robin});
...
MyTest::initData ()
{
    ...
    // Domain is box (r,z) in [0,1]x[-1,1]
    const Real dsize = 2.;
    const std::array<Real,AMREX_SPACEDIM> a_lo = {0., -dsize};
    const std::array<Real,AMREX_SPACEDIM> a_hi = {dsize, dsize};
    RealBox rb(a_lo, a_hi);
